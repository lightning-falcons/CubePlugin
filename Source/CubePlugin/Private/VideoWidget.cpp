...
void UVideoWidget::NativeConstruct()
{
	...
	if (GetWorld()->GetGameState<ADynamicGameState>()->PhotoImport)
	{
		...
		FString directoryToSearch = FString(GetWorld()->GetGameState<ADynamicGameState>()->FullImagePath.c_str());
		...
		filesInDirectory = ADynamicGameState::ExtractEvery(filesInDirectory, GetWorld()->GetGameState<ADynamicGameState>()->DownSamplePer * 3, GetWorld()->GetGameState<ADynamicGameState>()->NumberFrames);
